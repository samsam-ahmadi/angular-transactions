...
describe('Transactions list and details', () => {
    it('Transactions List', () => {
     ...
     cy.wait('@getTx').then((interception) => {
        ...
        // Trancsaction list
        const getamount = firstTransaction.currencyCode ===CurrencyCode.EUR ? firstTransaction.amount : (firstTransaction.amount * firstTransaction.currencyRate).toFixed(2);
        cy.get('.transactions-list').first().contains(getamount.toString());
        cy.get('.transactions-list').first().contains(firstTransaction.otherParty?.name ? firstTransaction.otherParty?.name : "Unknown");
        ...
        cy.get('h1').contains(firstTransaction.otherParty?.name ? firstTransaction.otherParty?.name : "Unknown");
